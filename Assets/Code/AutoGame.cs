...
public class AutoGame : MonoBehaviour {
    ...
    {
        string fileName = "Assets/Resources/locations.json";
        LoadJson(fileName);
    }
    ...
    public void LoadJson(string fileName)
    {
        using (StreamReader r = new StreamReader(fileName))
        {
            ...
            Locations items = JsonUtility.FromJson<Locations>(json);

            for (int i = 0; i < items.Value.Length; i++)
            {
                // List of arguments for shape instance
                // need to instantiate these now
                List<string> args = ExtractParameters(items.Value[i].ToString());
                GameObject toInstantiate = DetermineObject(args[2]);
                if (toInstantiate != null)
                {
                    GameObject newPlus = GameObject.Instantiate(toInstantiate, new Vector3(float.Parse(args[0])/2, float.Parse(args[1])/2, 0f), transform.rotation);
                }
            }
        }
    }
